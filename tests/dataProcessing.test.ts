...
describe('resolveItem (via getProcessedImageDataSync)', () => {
  ...
  test('非字串或空白 key 視為沒有，退回 episode 推導', () => {
    ...
    for (const bad of cases) {
      const [item] = getProcessedImageDataSync([
        { ...base, episode: 'mujica_9', filename: 'y.jpg', key: bad as never },
      ])
...
describe('storageHref', () => {
  test('優先取 storagePath', () => {
    expect(storageHref({ alt: '', storagePath: '/k.jpg', filename: 'f.jpg' })).toBe('/k.jpg')
  })

  test('無 storagePath 時退回 filename，再退回 file_name（legacy）', () => {
    expect(storageHref({ alt: '', filename: '/mygo/1/f.jpg' })).toBe('/mygo/1/f.jpg')
    expect(storageHref({ alt: '', file_name: '/legacy.jpg' })).toBe('/legacy.jpg')
    expect(storageHref({ alt: '' })).toBe('')
  })
})
